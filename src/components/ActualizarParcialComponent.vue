    ...
    <div class="container">
        ...
        <div class="modificar" v-if="estudiante.nombre">
            <form @submit.prevent="guardarEstudiante" class="form-estudiante">
                ...
                <input type="datetime-local" v-model="estudiante.fechaNacimiento" >

                <label>Provincia:</label>
                <input type="text" v-model="estudiante.provincia" >

                <label>Género:</label>
                <input type="text" v-model="estudiante.genero" >
...
<script>
import { actualizarParcialFachada, consultarPorIdFachada } from "../Client/MatriculaClient.js";
    ...
    methods: {
        async guardarEstudiante() {
            try {
                ...
                Object.keys(this.estudiante).forEach(key => {
                    if (this.estudiante[key] !== "" && this.estudiante[key] !== null) {
                        bodyParcial[key] = this.estudiante[key];
                    }
                });

                await actualizarParcialFachada(this.id_estudiante, bodyParcial);
                setTimeout(async () => {
